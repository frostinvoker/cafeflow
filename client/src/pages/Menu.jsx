...
export default function Menu() {
  ...
  const [editForm, setEditForm] = useState({
    _id: "", name: "", category: "", price: "", available: true,
  });
  ...
  const [addOnForm, setAddOnForm] = useState({ name: "", category: "", price: "", active: true });

  const [showEditAddOn, setShowEditAddOn] = useState(false);
  const [editAddOnForm, setEditAddOnForm] = useState({ _id: "", name: "", category: "", price: "", active: true });
  ...
  // Loads Addons
    useEffect(() => {
      ...
      (async () => {
        try {
          const res = await fetch("http://localhost:5000/api/addons", { credentials: "include" });
  ...
  // Handlers
  const openAddMenu = () => { setForm({ name: "", category: "", price: "" }); setShowAddMenu(true); };
  ...
  const handleNewItem = async (e) => {
    e.preventDefault();
    try {
      const body = { name: form.name.trim(), category: form.category, price: Number(form.price) || 0 };
  ...
  const openEditMenu = (item) => {
    setEditForm({
      ...
      price: String(item.price ?? ""), available: !!item.available,
    });
    setShowEdit(true);
  };
  ...
  const handleUpdateItem = async (e) => {
    ...
    try {
      ...
      if (Number.isNaN(priceNum) || priceNum < 0) throw new Error("Enter a valid price");
      const body = {
  ...
  const openDeleteConfirm = (item) => { setDeleteTarget(item); setShowDelete(true); };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      closeDeleteConfirm();
    } catch (err) { setError(err.message || "Failed to delete item"); }
  };
  ...
  const handleAddOnChange = (e) => {
    const { name, value, type, checked } = e.target;
    setAddOnForm((f) => ({ ...f, [name]: type === "checkbox" ? checked : value }));
  };
  ...
  const handleCreateAddOn = async (e) => {
    ...
    setError("");
    try {
      const body = { name: addOnForm.name.trim(), category: addOnForm.category, price: Number(addOnForm.price) || 0, active: !!addOnForm.active, applicableTo: "drink" };
      ...
      closeAddOnModal();
    } catch (err) { setError(err.message || "Failed to add add-on"); }
  };
  ...
  const handleEditAddOnChange = (e) => {
    const { name, value, type, checked } = e.target;
    setEditAddOnForm((f) => ({ ...f, [name]: type === "checkbox" ? checked : value }));
  };
  ...
  const handleUpdateAddOn = async (e) => {
    ...
    setError("");
    try {
      const body = { name: editAddOnForm.name.trim(), category: editAddOnForm.category, price: Number(editAddOnForm.price) || 0, active: !!editAddOnForm.active };
      const res = await fetch(`http://localhost:5000/api/addons/${editAddOnForm._id}`, {
        method: "PUT", headers: { "Content-Type": "application/json" }, credentials: "include",
        body: JSON.stringify(body),
      });
      const updated = await res.json().catch(() => null);
      if (!res.ok) throw new Error(updated?.message || "Failed to update add-on");
      setAddons((list) => list.map((x) => (x._id === updated._id ? updated : x)));
      closeEditAddOn();
    } catch (err) { setError(err.message || "Failed to update add-on"); }
  };
  ...
  if (!deleteAddOnTarget) return;
    try {
      const res = await fetch(`http://localhost:5000/api/addons/${deleteAddOnTarget._id}`, {
        method: "DELETE",
        credentials: "include",
      });
      const data = await res.json().catch(() => null);
  ...
  const renderMenuTable = (rows) => (
    <table className="menu" cellSpacing={0} cellPadding={0}>
      ...
      <tbody>
        {rows.map((it) => (
          <tr key={it._id} className="table-body">
            ...
            <td className={`availability ${it.available ? "available" : "unavailable"}`}>
              <span className="badge">{it.available ? "Available" : "Not Available"}</span>
            </td>
        ...
        {rows.length === 0 && (
          <tr><td colSpan={5} style={{ padding: "0.8rem" }}>No items in this category.</td></tr>
        )}
      </tbody>
    </table>
  );
      ...
      <tbody>
        {rows.map((a) => (
          <tr key={a._id} className="table-body">
            ...
            <td className={`availability ${a.active ? "available" : "unavailable"}`}>
              <span className="badge">{a.active ? "Available" : "Not Available"}</span>
            </td>
            <td className="buttons">
              <button className="edit" onClick={() => openEditAddOn(a)}>Edit</button>
              <button className="remove" onClick={() => openDeleteAddOnConfirm(a)}>Remove</button>
            </td>
          </tr>
        ))}
        {rows.length === 0 && (
          <tr><td colSpan={5} style={{ padding: "0.8rem" }}>No add-ons yet.</td></tr>
        )}
      </tbody>
    </table>
  );
  ...
  return (
    <div className="default-container">
      ...
      {/* FOR ADDON*/}
      {showAddOn && (
        <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) closeAddOnModal(); }}>
          <div className="modal">
            <h2>Add New Add-On</h2>
            <form onSubmit={handleCreateAddOn} className="modal-addon-form">
              <input name="name" value={addOnForm.name} onChange={handleAddOnChange} required placeholder="Add-On Name" />
              <select name="category" value={addOnForm.category} onChange={handleAddOnChange} required>
                <option value="" disabled>Select Category</option>
                {categoryOptions.map((u) => <option key={u} value={u}>{u}</option>)}
              </select>
              <input name="price" type="number" min="0" step="0.01" value={addOnForm.price} onChange={handleAddOnChange} required placeholder="Price (₱)" />
              <label className="checkbox-availability">
                <input type="checkbox" name="active" checked={!!addOnForm.active} onChange={handleAddOnChange} />
                Available
              </label>
              <div className="modal-actions">
                <button type="submit" className="primary">Add Add-On</button>
                <button type="button" className="secondary" onClick={closeAddOnModal}>Cancel</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showEditAddOn && (
        <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) closeEditAddOn(); }}>
          <div className="modal">
            <h2>Edit Add-On</h2>
            <form onSubmit={handleUpdateAddOn} className="modal-addon-form"> 
              <input name="name" value={editAddOnForm.name} onChange={handleEditAddOnChange} required placeholder="Add-On Name" />
              <select name="category" value={editAddOnForm.category} onChange={handleEditAddOnChange} required>
                <option value="" disabled>Select Category</option>
                {categoryOptions.map((u) => <option key={u} value={u}>{u}</option>)}
              </select>
              <input name="price" type="number" min="0" step="0.01" value={editAddOnForm.price} onChange={handleEditAddOnChange} required placeholder="Price (₱)" />
              <label className="checkbox-availability">
                <input type="checkbox" name="active" checked={!!editAddOnForm.active} onChange={handleEditAddOnChange} />
                Available
              </label>
              <div className="modal-actions">
                <button type="submit" className="primary">Save Changes</button>
                <button type="button" className="secondary" onClick={closeEditAddOn}>Cancel</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showDeleteAddOn && (
        <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) closeDeleteAddOnConfirm(); }}>
          <div className="modal">
            <h2>Confirm Removal</h2>
            <p>Are you sure you want to remove <strong>{deleteAddOnTarget?.name}</strong>?</p>
            <div className="modal-actions">
              <button className="remove" onClick={handleConfirmDeleteAddOn}>Yes, Remove</button>
              <button className="secondary" onClick={closeDeleteAddOnConfirm}>Cancel</button>
            </div>
          </div>
        </div>
      )}
