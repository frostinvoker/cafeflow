...
export default function Pos() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("http://localhost:5000/api/addons?active=true", {
          credentials: "include",
          signal: abort.signal,
        });
  ...
  const addConfiguredLine = () => {
    ...
    const chosen = selectableAddons.filter((a) => configAddonIds.includes(String(a._id)));
    const line = {
  ...
  return (
    <div className="pos-container">
      <div className="products-container">
        ...
        {loading ? (
          ...
          <div className="product-list">
            ...
            {items.length === 0 && <div style={{ padding: "1rem" }}>No items yet.</div>}
          </div>
        )}
      </div>
      ...
      <div className="checkout">
          ...
          {order.map((li) => {
            ...
            return (
                <div key={li.lineId} className="ordered-item">
                    ...
                    <div onClick={() => removeFromOrder(li.lineId)} className="remove-order">
                        <svg width="15" height="19" viewBox="0 0 15 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M14.5833 1.04167H10.9375L9.89583 0H4.6875L3.64583 1.04167H0V3.125H14.5833M1.04167 16.6667C1.04167 17.2192 1.26116 17.7491 1.65186 18.1398C2.04256 18.5305 2.57247 18.75 3.125 18.75H11.4583C12.0109 18.75 12.5408 18.5305 12.9315 18.1398C13.3222 17.7491 13.5417 17.2192 13.5417 16.6667V4.16667H1.04167V16.6667Z" fill="#FF4D4D"/>
                        </svg>
                    </div>
        ...
        <button className="checkout-btn" onClick={goToCheckout} disabled={order.length === 0}>
          Checkout
        </button>
      </div>
            ...
            <div className="modal-order-addons">
              ...
              <div className="addon-grid">
                {selectableAddons.map((a) => {
                  ...
                  return (
                    <label key={id} className="addon-chip">
                      <input
                        type="checkbox"
                ...
                {selectableAddons.length === 0 && <small>No add-ons available.</small>}
              </div>
            </div>

            <div className="modal-buttons">
              <button className="add-order" onClick={addConfiguredLine}>Add to Order</button>
              <button className="cancel-order" onClick={closeConfig}>Cancel</button>
            </div>
          </div>
        </div>
      )}
