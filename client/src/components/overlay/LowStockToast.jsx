...
export default function LowStockToast({ threshold = 3, pollMs = 30000, apiBase = "" }) {
  const [items, setItems] = useState([]);
  const [open, setOpen] = useState(false);
  function loadSeen() { try { return JSON.parse(localStorage.getItem("seenLowStock") || "{}"); } catch { return {}; } }
  ...
  useEffect(() => {
    ...
    const fetchLowStock = async () => {
      try {
        ...
        const res = await fetch(`${apiBase}/api/inventory/low-stock?threshold=${threshold}`, {
          credentials: "include",
          signal: ctrl.signal
        });
        if (!res.ok) return;
      ...
      <div className="lowstock-body">
        {items.map(it => (
          <div key={it._id} className="lowstock-line">
            <span>{it.name}</span>
            <span>{it.quantity} {it.unit || "pcs"} left</span>
          </div>
        ))}
      </div>
    </div>
