...
export default function Receipt() {
  const { checkoutId } = useParams(); // Get checkoutId from the URL
  ...
  useEffect(() => {
    const fetchCheckout = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/checkouts/${checkoutId}`, {
          credentials: "include",
        });
      ...
      <div className="receipt-container">
        <div className="receipt">
            ...
            {checkout.items.map((li) => {
              const addonsText =
                li.addons && li.addons.length > 0
                  ...
                  : "";
              const lineTotal =
                (li.price + (li.addons || []).reduce((s, a) => s + (a.price || 0), 0)) * (li.quantity || 1);
              return (
          ...
          <div className="buttons">
            ...
            <button className="gray-long-button" onClick={() => navigate("/pos")}>Back to POS</button>
          </div>
        </div>
      </div>
    </div>
